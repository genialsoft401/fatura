$(document).ready(function () {
    ...
    function loadDDI(selectedDDIs = {}) {
        fetch("assets/ajax/get_countries.php")
            ...
            .then(data => {
                ...
                const ddiDataForSelect2 = [{ id: '', text: 'DDI', selectedText: 'DDI' }].concat(
                    data.map(country => {
                        return {
                ...
                const ddiFields = ["telephone_ddi", "cellphone_ddi", "pref_telephone_ddi", "pref_cellphone_ddi"];
                ddiFields.forEach(fieldId => {
                    ...
                    field.select2({
                        ...
                        templateSelection: function (option) {
                            ...
                            const selectedItem = ddiDataForSelect2.find(item => item.id === option.id);
    ...
    function toggleFields() {
        ...
        if (isChecked) {
          $("#observations, #due_date,#numberCopys, #language, #currency, #payment_method").addClass("blocked-select").attr("tabindex", "-1");
        } else {
          $("#observations, #due_date, #numberCopys, #language, #currency, #payment_method").removeClass("blocked-select").removeAttr("tabindex");
        }
      }
    ...
    $("#btnFillContact").on("click", function () {
        ...
        $("#email").val("contato" + Math.floor(Math.random() * 1000) + "@teste.com");
