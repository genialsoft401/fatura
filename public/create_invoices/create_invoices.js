$(document).ready(function () {
  ...
  function addItemRow(item) {
    ...
    let is0 =
      taxVal == 0 || taxVal == "0" || taxVal == "exempt" || taxVal == "isento";
  ...
  function updateRetention(subtotalWithoutTax, totalTax) {
    ...
    if (retentionPercentage > 0) {
      ...
      $("#retention_value").text(
        formatCurrency(retentionValue, currencySymbol, currencyPosition),
      );
      ...
      success: function (response) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            window.location.href = "invoice.php?id=20260410/1/90";
          }
        });
      },
      error: function (xhr, status, error) {
