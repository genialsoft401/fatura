$(document).ready(function () {
    ...
    const checkModal = new bootstrap.Modal(document.getElementById('checkCollaboratorModal'));
    const linkModal = new bootstrap.Modal(document.getElementById('linkCollaboratorModal'));
    const createModal = new bootstrap.Modal(document.getElementById('createCollaboratorModal'));

    // Modais de ações
    const changeRoleModal = new bootstrap.Modal(document.getElementById('changeRoleModal'));
    const unlinkUserModal = new bootstrap.Modal(document.getElementById('unlinkUserModal'));
    ...
    $('#checkCollaboratorForm').on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (!resp.success) {
                    Swal.fire('Erro', resp.message || 'Falha ao verificar usuário.', 'error');
                    return;
                }
                ...
                if (resp.exists) {
                    ...
                    $('#linkCollaboratorForm input[name="username"]').val(d.username || '');
                    ...
                    if (resp.already_linked) {
                        ...
                        $('#linkCollaboratorForm button[type="submit"]').prop('disabled', true);
                    } else {
                        $('#linkUserInfo')
                            .removeClass('alert-warning')
                            .addClass('alert-info')
                            .text('Este colaborador já possui conta. Vamos apenas vincular à empresa.');
                        $('#linkCollaboratorForm button[type="submit"]').prop('disabled', false);
                    }
    ...
    $('#linkCollaboratorForm').on('submit', function (e) {
        ...
        postCreateCollaborator(fd)
            .done(function (resp) {
                if (resp.success) {
                    linkModal.hide();
                    Swal.fire('Sucesso', 'Colaborador vinculado à empresa.', 'success');
                    setTimeout(() => window.location.reload(), 600);
                } else {
                    $btn.prop('disabled', false).text(oldTxt);
                    Swal.fire('Erro', resp.message || 'Erro ao vincular colaborador.', 'error');
                }
            })
            .fail(function () {
                $btn.prop('disabled', false).text(oldTxt);
            ...
            .done(function (resp) {
                if (resp.success) {
                    ...
                    if (resp.data && resp.data.generated_password) {
                        Swal.fire('Colaborador criado', 'Senha temporária: ' + resp.data.generated_password, 'success');
                    } else {
                    ...
                    $btn.prop('disabled', false).text(oldTxt);
                    Swal.fire('Erro', resp.message || 'Erro ao criar colaborador.', 'error');
                }
            })
            .fail(function () {
                $btn.prop('disabled', false).text(oldTxt);
    ...
    $(document).on('click', '.js-unlink-user', function () {
        ...
        if (String(currentRole) === 'owner') {
            Swal.fire('Atenção', 'Não é possível remover um proprietário (owner) por aqui.', 'warning');
            return;
        }
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.success) {
                    ...
                    $btn.prop('disabled', false).text(oldTxt);
                    Swal.fire('Erro', resp.message || 'Erro ao remover colaborador.', 'error');
                }
            },
            error: function () {
                $btn.prop('disabled', false).text(oldTxt);
